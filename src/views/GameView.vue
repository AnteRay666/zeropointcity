    ...
    <div class="min-h-screen bg-[#0a0a0a] text-white p-4 md:p-8">
        ...
        <div class="max-w-7xl mx-auto flex flex-wrap justify-between items-center mb-8 gap-4">
            ...
            <div class="flex gap-4">
                <div class="bg-gray-900/50 border border-gray-800 rounded-lg px-4 py-2 text-center">
                    <div class="text-xs text-gray-500 uppercase">SSR 保底</div>
                    <div class="text-xl font-bold text-yellow-500">{{ 90 - pitySSR }}</div>
                </div>
                <div class="bg-gray-900/50 border border-gray-800 rounded-lg px-4 py-2 text-center">
                    <div class="text-xs text-gray-500 uppercase">SR 保底</div>
                    <div class="text-xl font-bold text-purple-500">{{ 10 - pitySR }}</div>
                </div>
        ...
        <div class="flex justify-center gap-4 md:gap-8 mb-12">
            <button @click="performGacha(1)" :disabled="isDrawing"
                class="group relative px-8 py-4 bg-black border border-blue-500/50 rounded-xl overflow-hidden transition-all hover:border-blue-400 disabled:opacity-50">
                <div class="absolute inset-0 bg-blue-500/5 group-hover:bg-blue-500/10 transition-colors"></div>
                <span class="relative text-blue-400 font-bold">单次招募</span>
            </button>

            <button @click="performGacha(10)" :disabled="isDrawing"
                class="group relative px-10 py-4 bg-black border border-purple-500/50 rounded-xl overflow-hidden transition-all hover:border-purple-400 disabled:opacity-50">
                <div class="absolute inset-0 bg-purple-500/5 group-hover:bg-purple-500/10 transition-colors"></div>
                <span class="relative text-purple-400 font-bold">十连招募</span>
            </button>
        </div>
        ...
        <div class="max-w-7xl mx-auto min-h-[400px]">
            <TransitionGroup name="gacha-list" tag="div" :class="[
                ...
                currentResult.length === 1 ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5'
            ]">
